 ...
 * @LastEditTime: 2023-02-19 22:17:07
-->
<template>
    <div id="gzGroupWarp" :class="wrapClass">
        <slot />
    </div>
</template>
...
function registerAutoGrid() {
    const wrap = document.querySelector("#gzGroupWarp")!;
    // 不兼容就不监听
...
function handleGrid() {
    // 自动时适应根据块的宽度判断设置grid
    const width = (document.querySelector("#gzGroupWarp") as HTMLElement)
        .offsetWidth;
    if (!props.grid) {
        if (!isDef(width)) {
            mapGrid.value = 4;
